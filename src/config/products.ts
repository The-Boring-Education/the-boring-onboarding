...
export const PRODUCT_CONFIGS: Record<string, OnboardingProductConfig> = {
  webapp: {
    ...
    fields: [
      ...
      createField('purpose', 'Purpose', 'multiselect', 3, {
        ...
        options: ['BUILDING_PROJECTS', 'LEARNING', 'NETWORKING', 'JOB_SEARCH'],
        prefill: {
    ...
    api: {
      endpoint: (userId: string) =>
        `${API_BASE}/api/v1/user/onbording?userId=${userId}`,
      method: 'POST',
  ...
  prepyatra: {
    ...
    api: {
      endpoint: () => `${API_BASE}/api/v1/prepyatra/onboarding`,
      method: 'POST',
  ...
  //   ],
  //   api: {
  //     endpoint: `${API_BASE}/api/v1/newproduct/onboarding`,
  //     method: 'POST',
